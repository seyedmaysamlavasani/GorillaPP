...
GS_READ_PAGE()
{
  ...
  memRep1 = mem(memReq2); 
  memRep = memRep1;
  //If we use memRep1 instead of  memRep, the LHS will be unconnected
  page.fanoutDegree = fanoutInPage(memRep1.data); 
  linkId = startLinkIndxInPage(memRep1.data);
  if (page.fanoutDegree <= 32) {
    fanoutDegree = page.fanoutDegree;
  } else {
    fanoutDegree =  page.fanoutDegree & 31;
  }
