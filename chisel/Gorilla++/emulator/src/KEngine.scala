...
class KEngine extends gComponentLeaf  (() => new KEngineIO_t)(() => new KEngineIO_t) (ArrayBuffer(("distanceFU", () => new distanceFU_in_t , () => new distanceFU_out_t), ("centeroidMem", () => new pointMemInput_t , () => new pointMemOutput_t), ("partialAccumulatorMem", () => new pointMemInput_t , () => new pointMemOutput_t), ("add", () => new FU_in_t , () => new FU_out_t)))  with include {
  val numOfThreads =  1  
  ...
  def  myOff = io.elements.find(_._1 == "off").getOrElse(elseV)._2
   ...
   val cMemOut = Vec (numOfThreads) {Reg(new pointMemOutput_t)}	//Global variable 
   ...
   val centeroidMemPort = new gOffBundleND(() => new pointMemInput_t, () => new pointMemOutput_t)
   ...
   val cMemIn1 = new pointMemInput_t
   val GS_RECEIVE_POINT = UFix(3)
   val GS_READ_CENTROID = UFix(4)
   val cMemIn2 = new pointMemInput_t
   ...
   val pMemIn3 = new pointMemInput_t
   val GS_EMIT = UFix(13)
...
addPortHadValidRequest(i) := !(vThread === UFix(i,5)) && (addPortHadValidRequest(i) || (UFix(i,5)===rThread && addPort.req.valid)/*(addPortHadReadyRequest && AllOffloadsReady && (UFix(i,5) === rThread))*/)
   ...
   pMemIn1.data.x:=UFix(0, width = 32) 
   pMemIn1.data.y:=UFix(0, width = 32) 
   pMemIn1.data.y:=UFix(0, width = 32) 
   pMemIn1.rw:=WRITE 
   cMemIn1.addr:=centeroidIndex(rThread) 
   cMemIn1.data:=inputReg(rThread).point 
   cMemIn1.rw:=WRITE 
   cMemIn2.addr:=centeroidIndex(rThread) 
   cMemIn2.rw:=READ 
   dfuin.in1:=cMemOut(rThread).data 
   ...
   partialAccumulatorMemPort.req.bits := MuxCase(UFix(0, 32),Seq( ((rThread != NONE_SELECTED && State(rThread) === GS_RECEIVE_CENTROID),pMemIn1) ,  ((rThread != NONE_SELECTED && State(rThread) === GS_UPDATE_PARTIAL_ACCUMULATE_READ),pMemIn2) ,  ((rThread != NONE_SELECTED && State(rThread) === GS_UPDATE_PARTIAL_ACCUMULATE_WRITE),pMemInGlobal(rThread)) ,  ((rThread != NONE_SELECTED && State(rThread) === GS_SEND_RESULTS),pMemIn3)))
  ...
  when (rThread != NONE_SELECTED && State(rThread) === WaitForOutputReady && io.out.ready) {
   ...
   closestCentroid(vThread):=cMemOut(vThread).data 
   }
   centeroidIndex(vThread):=centeroidIndex(vThread)+UFix(1, width = 32) 
   ...
   pMemInGlobal(vThread).addr:=closestIndex(vThread) 
   pMemInGlobal(vThread).rw:=WRITE 
   pMemOut(vThread) := partialAccumulatorMemPort.rep.bits
   State(vThread):=WaitForInputValid 
   } 
