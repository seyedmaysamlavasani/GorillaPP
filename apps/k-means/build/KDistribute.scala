...
class KDistribute (extCompName:String) extends gComponentLeaf  (() => new KEngineIO_t)(() => new KEngineIO_t) (ArrayBuffer())  (extCompName = extCompName + "__type__engine__MT__1__") with include {
  val numOfThreads = 1 
  ...
  def  myOff = io.elements.find(_._1 == "off").getOrElse(elseV)._2
   ...
   val int mode = Vec (numOfThreads) {Reg(new shared, resetVal=)}	//Global variable 
