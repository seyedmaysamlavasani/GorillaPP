...
class uint64_t extends UFix

object gPipe {
  def apply(latency:Int = 1, 
   ig: () => Chisel.Data = (() => UFix(width = 32))) = {
    val md = new gComponentMD(ig, ig, ArrayBuffer())
    (md, () => new gPipe(latency)).
...
class gTester (o: Component) extends Tester(o, Array(o.io)) {
  ...
  def printModuleStateCounters(moduleId: Int, 
   moduleName: String) = {
    val io = o.asInstanceOf[Top].io
    for ( i <- 0 until 16) {
      step(svars, ovars)
      svars(io.pcIn.valid) = Bool(true)
      svars(io.pcIn.bits.request) = Bool(true) 
      svars(io.pcIn.bits.pcType) = Pcounters.stateCounters
      svars(io.pcIn.bits.moduleId) = UFix(moduleId)
      svars(io.pcIn.bits.portId) = UFix(i) 
      step(svars, ovars)
      svars(io.pcIn.valid) = Bool(false)
      while(ovars(io.pcOut.valid).litValue().intValue() == 0) {
        step(svars, ovars)
      }
      println("PCREPORT: " + "stateCounters " + i + " " +
        moduleName + " received " + ovars(io.pcOut.bits.
         pcValue).litValue().intValue())
    }
  }

  def getStateCounters() = {
    val io = o.asInstanceOf[Top].io
    for ((name, id) <- Pcounters.moduleIDs) {
      printModuleStateCounters(id, name)
    }
  }
