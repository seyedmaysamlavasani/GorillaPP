...
class dramBank (extCompName:String) extends gComponentLeaf  (() => UFix(width = 32))(() => UFix(width = 32)) (ArrayBuffer())  (extCompName = extCompName + "__type__engine__MT__1__") with include {
  val numOfThreads = 1 
  ...
  def  myOff = io.elements.find(_._1 == "off").getOrElse(elseV)._2
   ...
   val outputReg = Vec(numOfThreads) {Reg(UFix(width = 32))}
   val GS_step1 = UFix(1)
   val GS_step2 = UFix(2)
   val GS_finish = UFix(3)
  //stateCounters
...
/**************************************************************/
  ...
  when (rThread != NONE_SELECTED && State(rThread) === WaitForOutputReady && io.out.ready) {
   ...
   outputReg(vThread):=inputReg(vThread)+UFix(1, width = 32) 
   } 
